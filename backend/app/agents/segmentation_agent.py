...
class SegmentationAgent:
    ...
    def is_healthy(self) -> bool:
        """Check if the segmentation agent is healthy"""
        return self.is_healthy_flag and self.model is not None
    
    async def segment_answers(self, processed_images: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for image_data in processed_images:
            ...
            try:
                ...
                segmented_results.append({
                    ...
                    "segmented_regions": answer_regions,
                    "total_questions": len(answer_regions),
                    "status": "success"
                })
                
            except Exception as e:
                logger.error(f"Error segmenting {image_data['original_path']}: {str(e)}")
                segmented_results.append({
                    "original_path": image_data["original_path"],
                    "segmented_regions": [],
                    "status": "error",
