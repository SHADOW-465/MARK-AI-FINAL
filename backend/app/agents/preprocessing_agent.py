...
class PreprocessingAgent(BaseAgent):
    ...
    async def process(self, state: GradingState) -> GradingState:
        ...
        if not file_path:
            state["error"] = "Image path not found in state"
            state["status"] = "error"
            return state
        ...
        try:
            ...
            if image is None:
                ...
                state["error"] = f"Could not load image: {file_path}"
                state["status"] = "error"
                return state
            ...
            state["preprocessed_image"] = binarized_image
            state["image_path"] = processed_path
            state["status"] = "preprocessing_complete"
            return state
